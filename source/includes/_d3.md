...
A d3 document can utilize an external dataset document as its own data. This option can be accessed under Data > Dataset > Use external.
...
A document may have mutiple internal datasets that can be added under **Data > Dataset > Add new**.

When adding a new dataset, all current data structure will be moved to *data['data0']*. A dataset data can be accessed using the format *data['datasetName']*.
This datasetName has a default name as *dataN* with N being the number of datasets that exist in the document. The default *datasetName* can be changed by clicking on the datasetName field to the left. 
datasetName convention follows a file name convention which includes alphabets, numbers, spaces, dash and underscore.

A currently viewing dataset can be removed using the option **Data > Dataset > Remove current**. This option is unavailable if the document has only 1 dataset.

A document can only have a maximum of 10 datasets.

When switching to an external dataset, all current datasets will be removed. Upon switching back to using internal dataset, all data will be copied over as the sole and only dataset for the document.

Besides, Auto properties detection for axis is applied on the first or primary dataset only.
